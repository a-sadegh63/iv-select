...
jQuery(document).on('click', '.iv-select-topbar', function(e) {
    var iv_width = jQuery(this).outerWidth();
    var options_container = jQuery(this).iv_findElement( iv_elements.options_container_el );
    options_container.css( 'max-width', iv_width );
    options_container.css( 'top', jQuery(this).offset().top + jQuery(this).outerHeight() );
    options_container.css( 'left', jQuery(this).offset().left );
});

jQuery(document).on('keydown', '.iv-select-view', function(e) {
        ...
        if ( options_container.children('.iv-option-focused').length != 0 ) {
            ...
            if ( options_container[0].scrollTop > 0 ) {
                options_container.find(':visible').each( function () {
                    if ( jQuery(this).offset().top - container_top <= jQuery(this).outerHeight() ) {
                        new_focus_el = jQuery(this);
                    }
                });
            } else {
                new_focus_el = options_container.find(':visible:last');
...
jQuery(document).on('keyup', '.iv-select-view', function(e) {
    var key_code = e.keyCode || e.which;
    if ( key_code == 40 || key_code == 38 ) {
        return;
    }
...
jQuery(document).on('click', '.iv-select', function(e) {
    var target_el = jQuery(e.target);
    if ( 
...
function ivDropOptionsDown( main_el ) {
    ...
    jQuery('.iv-select-options').not(options_container[0]).hide(); //hide other options containers
...
jQuery.propHooks.disabled = {
    set: function ( iv_select, prop_value ) {
        if ( jQuery(iv_select).is_ivSelect() ) {
            if ( prop_value ) {
                jQuery( iv_select ).iv_findElement( iv_elements.view_el ).css( 'background-color', '#f8f8f8' );
                jQuery( iv_select ).iv_findElement( iv_elements.search_el ).prop( 'disabled', true );
            } else {
                jQuery( iv_select ).iv_findElement( iv_elements.view_el ).css('background-color', 'unset');
                jQuery( iv_select ).iv_findElement( iv_elements.search_el ).prop( 'disabled', false );
            }
        }
    }
};

jQuery(document).on( 'mouseenter', '.iv-select-options option', function(e) {
    jQuery(this).attr( 'title', jQuery(this).text() );
});

jQuery(document).on('click', '.iv-select-options option', function(e) {
    var target_el = jQuery(e.target);
    var value_el = target_el.iv_findElement( iv_elements.value_el );
...
jQuery(document).on('click', '.iv-del-button', function(e) {
    var target_el = jQuery(e.target);
...
jQuery(document).on('keyup', 'input.iv-select-search', function(e) {
    var target = jQuery(e.target);
    if ( jQuery.inArray(e.keyCode, [37, 38, 39, 40, 9, 13, 16, 17, 18, 20]) != -1 ) {
        return;
    }
    ...
    const result = options.filter(index => jQuery(options[index]).text().toLowerCase().indexOf(search.toLowerCase()) > -1);
    if ( result.length != 0 ) {
        options.hide();
...
jQuery(document).on('click', function(e) {
    if (
        jQuery('.iv-select:hover').length == 0 &&
        jQuery('.iv-select-options:hover').length == 0 &&
        jQuery('.iv-select-view:hover').length == 0 &&
        jQuery('.iv-selected-item:hover').length == 0 &&
        jQuery('.iv-selected-item:hover').length == 0 &&
        jQuery('.iv-del-button:hover').length == 0 &&
        jQuery('.iv-select-search:hover').length == 0
    ) {
        jQuery('.iv-select-options').hide(200);
        jQuery('.iv-select').css({
            'border-bottom-right-radius': '',
            'border-bottom-left-radius': '',
        });
        jQuery('input.iv-select-search').map( function () {
            if ( jQuery(this).val() != '' ) {
                jQuery(this).val('');
                jQuery(this).trigger('keyup');
            }
        });
        jQuery('.iv-select').css( 'min-width', 'unset' );
    }
    if ( jQuery(e.target).hasClass('iv-tooltip') ) {
        jQuery('.iv-tooltip').hide('fade');
    }
    jQuery('.iv-option-focused').each( function() {
        jQuery(this).removeClass('iv-option-focused');
    });
    jQuery('.iv-select-options').children().css( 'pointer-events', 'unset' );
});

function addIvItem(item_text, item_val) {
    if ( item_text == '' ) return '';
    //add container node
    var container_node = jQuery('<div/>').attr({
    ...
    var text_node = jQuery('<span/>').attr({
    ...
    //add delete button
    var btn_node = jQuery('<button/>').attr({
        class: 'iv-del-button',
        ...
        options.each(function() {
            if (Array.isArray(value)) {
                if (value.indexOf(jQuery(this).val()) != -1) {
                    jQuery(this).addClass('w3-dark-gray');
                    value_text.push(addIvItem(jQuery(this).text(), jQuery(this).val()));
                    jQuery(this).prop("selected", true);
                    value_option += jQuery(this)[0].outerHTML;
                } else {
                    jQuery(this).prop("selected", false);
                    jQuery(this).removeClass('w3-dark-gray');
                }
            } else {
...
jQuery.fn.extend({
    ...
    function({
        ...
        cloned_iv.options_el.each(function() {
            jQuery(this).css( option_style );
            jQuery(this).addClass( option_class );
        });
    ...
    iv_setInvalid: function( err_message ) {
        ...
        text_el.after( 
            jQuery('<span style="position:absolute;background-color:rgb(30, 29, 34);color:white;' + 
              'font-size:13px;padding:18px;width:' + view_width + 'px;min-width:250px"' + 
              ' class="iv-tooltip w3-round-large">' + err_message + '</span>') 
        );
        this.on( 'invalid', function () {
            var text_el = jQuery(this).iv_findElement( iv_elements.view_el );
            if ( text_el.next('span.iv-tooltip').length != 0 ) {
    ...
    function({
        ...
        this.each(function() {
            ...
            var options_container = jQuery('<div/>').attr({
            ...
            options.each(function() {
                jQuery(this).addClass( iv_settings.options_el.classes );
                jQuery(this).css( iv_settings.options_el.styles );
                jQuery(this).addClass( args.option_class );
                jQuery(this).css( args.option_style );
            });
